...
class Car:
  ...
  def controls_update(self, CS: car.CarState, CC: car.CarControl, CC_SP: custom.CarControlSP):
    ...
    if self.sm.all_alive(['carControl']):
      ...
      model_age_nanos = now_nanos - self.sm.logMonoTime['modelV2']
      model = self.sm['modelV2'] if self.sm.all_checks(['modelV2']) and 0 <= model_age_nanos <= MODEL_MAX_AGE_NS else None
      self.last_actuators_output, can_sends = self.CI.apply(CC, convert_carControlSP(CC_SP), now_nanos, model=model)
