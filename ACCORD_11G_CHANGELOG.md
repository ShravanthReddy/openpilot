...
- **Gained: confidence-gated LaneCentering** (StarPilot port; params `LaneCentering` BOOL /
  `LaneCenteringE2EAuthority` FLOAT) — biases model curvature toward detected lane center;
  the corner-cutting experiment. ENABLED per user (LaneCentering=1).
- Also: Honda B-gear drivable, dash-lane display slew-limit + MADS cluster indication fixes,
  ACC command refactor (reviewed field-by-field: CAN-FD brake path byte-equivalent), branch
  switcher/updater resilience.

### (dev) Stop-Commit Assist — finish stops at empty red lights
- **Problem (9-drive analysis):** with no lead, RDF brakes hard (−2.2..−2.6) for red lights but
  starts late and arrives at the line still carrying 10–20 mph → driver takeovers (9 in ~2.1 h).
  With a lead present, stops are already excellent (7/8 flawless) — untouched.
- **Fix:** param `StopCommitGain` (FLOAT, 0=off, clamp 1.0–1.5): when in e2e mode with NO lead
  and the model has held ≥1 s of committed braking whose trajectory projects to v=0 within ~6 s
  (a stop, not a slowdown), scale its decel by the gain. Never creates braking, total capped at
  −3.0, jerk-limited (1.5 up/3.0 down), fades below ~8 mph, stands down instantly for leads.
- **Validated:** replayed against recorded red-light and curve profiles (assist engages
  mid-approach on stops, near-zero on curve slowdowns); 200k-state invariant sweep clean.
- **Files:** `openpilot/selfdrive/controls/lib/longitudinal_planner.py`, `openpilot/common/params_keys.h`

### (dev) MaxCruiseAccel param + trajectory-init bugfix (`e470fc5f98`)
- `MaxCruiseAccel` (0=stock, 1.2–2.0): rescales the cruise accel curve peak (community pair
  with E2eSpeedBias; successor to retired July `3eaf176`). Set to 1.9 per user preference.
- Bugfix: trajectory zeroing had slipped inside the param reader (one zeroed plan msg per ~5 s
  since `f2e73bdcb`; aTarget unaffected).

### (dev) E2E Speed Bias red-light gate (`e62e3f52c`)
- Log-measured leak: shallow approaches kept partial bias ~6 s (3 of 9 takeovers) → fade now
  zero by −0.15 and a >1.5 s sustained-decel trend suppresses bias until clear re-acceleration.

### (dev) E2E Speed Bias — full-e2e speed keeping
- **Problem:** with DEC off (full e2e), the model doesn't know the speed limit (2s history) and
  bleeds speed / won't climb to set speed on empty roads.
- **Fix:** param `E2eSpeedBias` (FLOAT, default 0=off, clamp 0–0.3): small accel added to the
  model's desiredAcceleration in e2e mode; fades to zero by −2× bias (stops always honored),
  suppressed when model says shouldStop, capped at set speed by the MPC min(). Live-tunable
  (~5s param re-read). Design from community (r/Comma_ai "longitudinal lever", narkeeso), which
  MVL intends to upstream; invariants verified over 200k random states.
- **Files:** `openpilot/selfdrive/controls/lib/longitudinal_planner.py`, `openpilot/common/params_keys.h`

## ⚡ DEV MOVE (Aug 09, same day) — `accord-11g-dev-202608`

User chose MVL's dev base (their historical norm, and required to run the new Deep-RL
models — RDF needs the v18 catalog + newer tinygrad, incompatible with release 202607).
Same two opendbc patches re-applied (`af7be981`). `accord-11g-202607` remains the stable
fallback: checkout + reboot. Dev extras now on-car: 23+ Accord steering logic, cluster
leadcar/lanelines display, Bosch C alphalong redesign. MVL: "very-experimental."
