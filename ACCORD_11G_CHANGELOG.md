...
- **Fork:** `ShravanthReddy/openpilot` @ branch `accord-11g-smooth` (+ submodule `ShravanthReddy/opendbc`)
- **Base:** MVL `sp-honda-dev-202606` @ `500e200` (opendbc base @ `4de1092`)
...
- **Verified:** 981 focused Honda/opendbc tests pass (69 expected skips), ruff clean, exact stock checksum vectors pass, independent Codex review and Claude Fable review both conditionally pass. On-device full build and exact-Accord interface test pass; controller integration confirms zero new IDs with the param off and bus-0-only 50/10/5/1 Hz output under an in-memory test enable. Parked param-on cluster canary remains required before supervised driving.
